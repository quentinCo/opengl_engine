...
void lightCulling(int threadIndex)
{
	...
	for(int i = 0; i < lightsToHandle; ++i)
	{
		...
		vec3 pointLightCoords = (uViewMatrix * vec4(pointLights[i].position)).xyz;
...
void main()
{
	...
	if(threadIndex == 0)
	{
		...
		depthMinFloat = clipSpaceToViewSpace(vec4(0, 0, uintBitsToFloat(depthMinInt), 1)).z;
		depthMaxFloat = clipSpaceToViewSpace(vec4(0, 0, uintBitsToFloat(depthMaxInt), 1)).z;
