...
Renderer::Renderer(Renderer&& o)
	...
	uKe(o.uKe), programBlurPassOnX (std::move(o.programBlurPassOnX)), programBlurPassOnY(std::move(o.programBlurPassOnY)), uInitTexOnX (o.uInitTexOnX),
	uInitTexOnY(o.uInitTexOnY), uTextureSizeXPass (o.uTextureSizeYPass), uTextureSizeYPass(o.uTextureSizeYPass),
...
Renderer& Renderer::operator=(Renderer&& o)
{
	...
	programBlurPassOnX = std::move(o.programBlurPassOnX);
	programBlurPassOnY = std::move(o.programBlurPassOnY);
	if (bufferBlurred) glDeleteTextures(1, &bufferBlurred);
	bufferBlurred = o.bufferBlurred;
	o.bufferBlurred = 0;
	...
	uInitTexOnX = o.uInitTexOnX;
	uInitTexOnY = o.uInitTexOnY;
	uTextureSizeXPass = o.uTextureSizeXPass;
	uTextureSizeYPass= o.uTextureSizeYPass;
...
{
	programBlurPassOnX = glmlv::compileProgram({ shaderDirectory / "postProcess" / "blurOnX.cs.glsl" });
	programBlurPassOnY = glmlv::compileProgram({ shaderDirectory / "postProcess" / "blurOnY.cs.glsl" });
	...
	uInitTexOnX = glGetUniformLocation(programBlurPassOnX.glId(), "uInitTex");
	uTextureSizeXPass = glGetUniformLocation(programBlurPassOnX.glId(), "uWindowDim");

	uInitTexOnY = glGetUniformLocation(programBlurPassOnY.glId(), "uInitTex");
	uTextureSizeYPass = glGetUniformLocation(programBlurPassOnY.glId(), "uWindowDim");
}
...
{
	postProcessBlurPassOnX(tex);
	postProcessBlurPassOnY(bufferBlurredTexPass1);
}


//-- POST PROCESS DIRECTION BLUR PASS -

void Renderer::postProcessBlurPassOnX(GLuint tex)
{
	programBlurPassOnX.use();

	glUniform2fv(uTextureSizeXPass, 1, glm::value_ptr(glm::vec2(windowWidth, windowHeight)));

	glBindImageTexture(0, bufferBlurredTexPass1, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex);
	glUniform1i(uInitTexOnX, 0);
...
void Renderer::postProcessBlurPassOnY(GLuint tex)
{
	programBlurPassOnY.use();

	glUniform2fv(uTextureSizeYPass, 1, glm::value_ptr(glm::vec2(windowWidth, windowHeight)));

	glBindImageTexture(0, bufferBlurred, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex);
	glUniform1i(uInitTexOnY, 0);
